...
- `docs/custom-pake-overlay-features.md` — feature map кастомного overlay-слоя над Pake без привязки к конкретной реализации;
...
- `same-window-routes.js` — управляет табами, навигацией и общими overlay-патчами внутри текущего окна, удерживая ссылки текущего домена в webview и отдавая внешние домены в браузер.
- `account-manager-routes.js` — отдельный injected-слой для экрана доменов, списка аккаунтов и добавления нового домена.
- `dist/empty-account.html` — локальная пустая стартовая страница, которая показывается, когда в native store ещё нет ни одного домена; на ней есть только кнопка входа в account-manager overlay.

Правило загрузки webview:

- если доменов нет, Rust отдаёт локальный `empty-account.html`;
- если активный домен есть, Rust подставляет его `base_url` ещё до создания main `WebviewWindow`;
